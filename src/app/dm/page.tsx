...
import { Send, Settings, Users, MessageSquare, Paperclip, X } from 'lucide-react';
...
function getChatLabel(dm: DMConversation): string {
  if (dm.isGroup) return dm.groupName ?? 'グループ';
  return dm.participants[0]?.displayName ?? 'Chat';
}

function getChatInitial(dm: DMConversation): string {
  if (dm.isGroup) return dm.groupName?.[0] ?? 'G';
  return dm.participants[0]?.username[0].toUpperCase() ?? '?';
}
...
export default function ChatPage() {
  ...
  const personalChats = conversations.filter(d => !d.isGroup);
  const groupChats = conversations.filter(d => d.isGroup);
  const allChats = [...personalChats, ...groupChats];
  const activeChat = allChats.find(d => d.id === activeId);
  ...
  // Fetch conversations on mount (requires auth)
  useEffect(() => {
    if (!isAuthenticated) return;
    fetchMyConversations(user.id).then(convs => {
      setConversations(convs);
      if (convs.length > 0) {
        setActiveId(convs[0].id);
        setHoveredId(convs[0].id);
      }
    });
  }, [isAuthenticated, user.id]);

  // Fetch messages + realtime subscription when conversation changes
  useEffect(() => {
    if (!activeId) return;
    fetchDMMessages(activeId).then(setMessages);
    ...
    const sub = supabase
      ...
      .on(
        ...
        async (payload) => {
          ...
          const msg = await fetchOneDMMessage(newId);
          if (msg) {
    ...
    const handle = (e: KeyboardEvent) => {
      ...
      if (mode === 'list') {
        if (e.key === 'j' || e.key === 'ArrowDown') {
          e.preventDefault();
          const hIdx = allChats.findIndex(d => d.id === hoveredId);
          if (hIdx < allChats.length - 1) setHoveredId(allChats[hIdx + 1].id);
        } else if (e.key === 'k' || e.key === 'ArrowUp') {
          e.preventDefault();
          const hIdx = allChats.findIndex(d => d.id === hoveredId);
          if (hIdx > 0) setHoveredId(allChats[hIdx - 1].id);
        } else if (e.key === 'Enter') {
          e.preventDefault();
  ...
  }, [mode, activeId, hoveredId, allChats.length]);
  ...
  const handleSend = async () => {
    ...
    const realId = await insertDMMessage({
      conversationId: activeId,
      authorId: user.id,
    ...
    <div style={{ background: 'var(--bg-primary)' }} className="flex h-full">
      ...
      >
        <div style={{ padding: '12px 12px 8px', borderBottom: '1px solid var(--border-subtle)' }} className="flex items-center gap-2">
          ...
          />
          <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600 }}>Chat</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-3">
          ...
          {!isAuthenticated && (
            <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', padding: '4px 12px' }}>ログインが必要です</p>
          )}
          {isAuthenticated && allChats.length === 0 && (
            <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', padding: '4px 12px' }}>DMはまだありません</p>
          )}
          {personalChats.map(dm => {
            const isActive = dm.id === activeId;
            const isCursor = mode === 'list' && dm.id === hoveredId;
            return (
              <div
                key={dm.id}
                tabIndex={0}
                onClick={() => { setActiveId(dm.id); setHoveredId(dm.id); setMode('open'); }}
                style={{
              ...
              >
                ...
                <span className="flex-1 truncate">{getChatLabel(dm)}</span>
                {dm.unreadCount > 0 && (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div style={{ borderBottom: '1px solid var(--border)', padding: '10px 16px', flexShrink: 0 }} className="flex items-center gap-2">
          ...
          {activeChat && !activeChat.isGroup && (
            <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>@{activeChat.participants[0]?.username}</span>
          )}
          {activeChat?.isGroup && (
            <span style={{ color: 'var(--text-muted)', fontSize: '0.75rem' }}>{activeChat.participants.length + 1} 人</span>
          )}
          <div className="ml-auto flex items-center gap-1" style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>
            ...
            <span style={{ marginLeft: 8 }} className="kbd">g f</span><span>Forum</span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3" tabIndex={0} style={{ outline: 'none' }}>
          {!activeId && (
            <div className="flex items-center justify-center h-full" style={{ color: 'var(--text-muted)' }}>
              <p className="text-sm">{isAuthenticated ? '会話を選択してください' : 'ログインが必要です'}</p>
            </div>
          )}
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '12px 16px', flexShrink: 0 }}>
          ...
          >
            ...
            <textarea
              ...
              placeholder={activeChat ? `${getChatLabel(activeChat)} にメッセージを送る... (Ctrl+Enter で送信)` : 'メッセージ'}
              rows={1}
              style={{
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.7rem', marginTop: 4 }}>
            ...
            <span className="kbd">j</span>/<span className="kbd">k</span> 選択 &nbsp;
